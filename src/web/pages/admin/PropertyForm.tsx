		...
		<div style={{ minHeight: "100vh", background: C.dark, color: C.light, padding: "32px clamp(16px,4vw,48px) 64px", marginTop: "-80px" }}>
			<div style={{ maxWidth: isNew ? "800px" : "1200px", margin: "0 auto" }}>
				<div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "28px", flexWrap: "wrap", gap: "12px" }}>
					<div>
						<Link href="/admin">
							<a style={{ color: C.light, fontFamily: "Nunito, sans-serif", fontSize: "0.8rem", textDecoration: "none" }}>← Назад</a>
						</Link>
						<h1 style={{ fontFamily: "JUN, Georgia, serif", fontSize: "1.75rem", margin: "8px 0 0", fontWeight: 500 }}>
							{isNew ? "Новый объект" : `Объект ${id}`}
						</h1>
					</div>
					<button type="button" disabled={saving} onClick={handleSave} style={{ padding: "12px 24px", background: C.accent, color: C.light, border: "none", borderRadius: "8px", fontFamily: "Nunito, sans-serif", fontWeight: 600, cursor: saving ? "wait" : "pointer", opacity: saving ? 0.7 : 1 }}>
				...
				<div style={{ display: "flex", gap: "32px", flexWrap: "wrap", alignItems: "flex-start" }}>
					...
					{!isNew && (
						<aside style={{ flex: "0 0 320px", maxWidth: "100%" }}>
							<h2 style={{ fontFamily: "JUN, Georgia, serif", fontSize: "1.1rem", margin: "0 0 16px", color: C.light }}>История</h2>
							<div style={{ borderLeft: `2px solid ${C.burgundy}`, paddingLeft: "16px" }}>
								{history.length === 0 ? (
									<p style={{ fontFamily: "Nunito, sans-serif", fontSize: "0.85rem", color: "#FFFEF9" }}>Нет записей</p>
								) : (
									history.map((h) => (
										<div key={h.id} style={{ marginBottom: "20px", position: "relative" }}>
											...
											<p style={{ fontFamily: "Nunito, sans-serif", fontSize: "0.65rem", color: C.light, margin: "0 0 4px", textTransform: "uppercase", letterSpacing: "0.08em" }}>
												{h.event} · {new Date(h.createdAt).toLocaleString("ru-RU")}
											</p>
											{h.description && (
												<p style={{ fontFamily: "Nunito, sans-serif", fontSize: "0.85rem", margin: "0 0 4px", lineHeight: 1.4 }}>{h.description}</p>
											)}
											{h.price != null && (
												<p style={{ fontFamily: "Nunito, sans-serif", fontSize: "0.75rem", color: "#FFFEF9", margin: 0 }}>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
	return (
		<div style={{ marginBottom: "28px", padding: "20px", background: "rgba(255,254,249,0.03)", border: `1px solid ${C.burgundy}`, borderRadius: "10px" }}>
			<h2 style={{ fontFamily: "Nunito, sans-serif", fontSize: "0.7rem", textTransform: "uppercase", letterSpacing: "0.12em", color: C.light, margin: "0 0 16px" }}>{title}</h2>
			{children}
		</div>
	);
}
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
	return (
		<div style={{ marginBottom: "14px" }}>
			<label style={{ display: "block", fontFamily: "Nunito, sans-serif", fontSize: "0.75rem", color: "#FFFEF9", marginBottom: "6px" }}>{label}</label>
			{children}
		</div>
	);
}
...
function Toggle({ label, checked, onChange }: { label: string; checked: boolean; onChange: (v: boolean) => void }) {
	return (
		<label style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "14px", cursor: "pointer", fontFamily: "Nunito, sans-serif", fontSize: "0.85rem" }}>

 ...
 *
 * Do not rewrite `realClientPageUrl` — Flat.show only loads apartments for
 * allowlisted client pages (Visarteam / Central MG / the Parkline tour).
 */
export const FLATSHOW_INTERCEPT_SCRIPT = `(function(){
  ...
  function hideLeadModal(){
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      if (hasPhone || /request a call|phone number|заказать звонок/.test(txt)) {
...
export async function fetchFlatshowEmbedHtml(
  ...
  const res = await fetch(url, {
    headers: {
      ...
      "User-Agent": "Mozilla/5.0 (compatible; SitboInvestEmbed/1.0)",
    },
  });
